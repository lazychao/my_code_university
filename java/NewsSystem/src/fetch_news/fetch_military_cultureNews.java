...
public class fetch_military_cultureNews implements fetchNews{
    public void fetch() throws IOException {
        Document index_doc = Jsoup.connect("http://news.sohu.com/").get();//获取首页的html
        // 2.jsoup获取新闻<a>标签
        Elements  newsATags = index_doc.select("div.content-military-culture")
                .select("div.main")
        ...
        int count=mapper.queryTotal("military_culture");//获取当前section已有的新闻总数
...
//                url=url.substring(2);
//            }
//            if( url.indexOf("http://www.sohu.com/a/")<0
//                    &&url.indexOf("https://www.sohu.com/a/")<0
//                    &&url.indexOf("www.sohu.com/a/")<0)//防止混入其他网址
//                continue;
            url="http:"+url;
            System.out.println(url);
            String title=element.attr("title");
            //打开每一条新闻的url  里面有文本和图片
            if (mapper.checkTitle(title,"military_culture")!=null){
            ...
            News military_culture_news=new News();
            military_culture_news.setId_news(count);
            military_culture_news.setDate_time(date_time);
            military_culture_news.setImg_path(img_path);
            military_culture_news.setNews_title(title);
            military_culture_news.setContent(content);
            mapper.insertNews(military_culture_news,"military_culture");//插入一条记录
            //System.out.println("id:"+id+'\n'+"text:"+text+"img_path"+img_path);

        }
        System.out.println("ss:"+count);
        mapper.updateTotal(count,"military_culture");
        sqlsession.commit();
    }
    ...
    public String downloadImg(Document news_doc,int count) throws IOException{
        ...
        String img_name="military_culture"+count+".png";

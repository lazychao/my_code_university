...
public class fetch_digitalNews implements fetchNews{
    public void fetch() throws IOException {
        Document index_doc = Jsoup.connect("http://news.sohu.com/").get();//获取首页的html
        // 2.jsoup获取新闻<a>标签
        Elements  newsATags = index_doc.select("div.content-it-digital")
                .select("div.main")
        ...
        for (Element element:newsATags){
            ...
            News digital_news=new News();
            digital_news.setId_news(count);
            digital_news.setDate_time(date_time);
            digital_news.setImg_path(img_path);
            digital_news.setNews_title(title);
            digital_news.setContent(content);
            mapper.insertNews(digital_news,"digital");//插入一条记录
            //System.out.println("id:"+id+'\n'+"text:"+text+"img_path"+img_path);

        }
        System.out.println("ss:"+count);
        mapper.updateTotal(count,"digital");
        sqlsession.commit();
    }
    ...
    public String downloadImg(Document news_doc,int count) throws IOException{
        ...
        String img_name="digital"+count+".png";

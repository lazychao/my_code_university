...
public class fetch_financeNews implements fetchNews{
    public void fetch() throws IOException {
        Document index_doc = Jsoup.connect("http://news.sohu.com/").get();//获取首页的html
        // 2.jsoup获取新闻<a>标签
        Elements  newsATags = index_doc.select("div.content-business-finance")
                .select("div.main")
        ...
        int count=mapper.queryTotal("finance");//获取当前section已有的新闻总数
        ...
        for (Element element:newsATags){
            ...
            if (mapper.checkTitle(title,"finance")!=null){
            ...
            News finance_news=new News();
            finance_news.setId_news(count);
            finance_news.setDate_time(date_time);
            finance_news.setImg_path(img_path);
            finance_news.setNews_title(title);
            finance_news.setContent(content);
            mapper.insertNews(finance_news,"finance");//插入一条记录
            //System.out.println("id:"+id+'\n'+"text:"+text+"img_path"+img_path);

        }
        System.out.println("ss:"+count);
        mapper.updateTotal(count,"finance");
        sqlsession.commit();
    }
    ...
    public String downloadImg(Document news_doc,int count) throws IOException{
        ...
        String img_name="finance"+count+".png";

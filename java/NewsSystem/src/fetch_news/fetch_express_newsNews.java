...
public class fetch_express_newsNews implements fetchNews{
    public void fetch() throws IOException {
        Document index_doc = Jsoup.connect("http://news.sohu.com/").get();//获取首页的html
        // 2.jsoup获取新闻<a>标签
        Elements newsATags = index_doc.select("div.contentA")
                .select("div.main")
                .select("div.main-modA")
                .select("div.main-right") //用于存储标签集合
                .select("div.focus-news")
                .select("div.list16")
                .select("ul")
                .select("li")
                .select("a");
        ...
        int count=mapper.queryTotal("express_news");//获取当前section已有的新闻总数
        ...
        for (Element element:newsATags){
            ...
            &&url.indexOf("https://www.sohu.com/a/")<0))//防止混入其他网址
                //第三个条件花了好多时间才知道要加，，不同栏目的新闻有的是http有的是https惊了
                continue;
            String title=element.attr("title");
            //打开每一条新闻的url  里面有文本和图片
            if (mapper.checkTitle(title,"express_news")!=null){
            ...
            News express_news_news=new News();
            express_news_news.setId_news(count);
            express_news_news.setDate_time(date_time);
            express_news_news.setImg_path(img_path);
            express_news_news.setNews_title(title);
            express_news_news.setContent(content);
            mapper.insertNews(express_news_news,"express_news");//插入一条记录
            //System.out.println("id:"+id+'\n'+"text:"+text+"img_path"+img_path);

        }
        System.out.println("ss:"+count);
        mapper.updateTotal(count,"express_news");
        sqlsession.commit();
    }
    ...
    public String downloadImg(Document news_doc,int count) throws IOException{
        ...
        String img_folder="C:\\Users\\Dell\\Documents\\idea_projects\\NewsSystem\\web\\img_news\\";//保存下载图片文件夹
        String img_name="express_news"+count+".png";
        String img_path=img_folder+img_name;//保存图片路径

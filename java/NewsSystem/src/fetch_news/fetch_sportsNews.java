...
public class fetch_sportsNews implements fetchNews{
    public void fetch() throws IOException {
        Document index_doc = Jsoup.connect("http://news.sohu.com/").get();//获取首页的html
        // 2.jsoup获取新闻<a>标签
        Elements  newsATags = index_doc.select("div.content-sports-yule")
                .select("div.main")
        ...
        int count=mapper.queryTotal("sports");//获取当前section已有的新闻总数
        ...
        for (Element element:newsATags){
            ...
            if (mapper.checkTitle(title,"sports")!=null){
            ...
            News sports_news=new News();
            sports_news.setId_news(count);
            sports_news.setDate_time(date_time);
            sports_news.setImg_path(img_path);
            sports_news.setNews_title(title);
            sports_news.setContent(content);
            mapper.insertNews(sports_news,"sports");//插入一条记录
            //System.out.println("id:"+id+'\n'+"text:"+text+"img_path"+img_path);

        }
        System.out.println("ss:"+count);
        mapper.updateTotal(count,"sports");
        sqlsession.commit();
    }
    ...
    public String downloadImg(Document news_doc,int count) throws IOException{
        ...
        String img_name="sports"+count+".png";
